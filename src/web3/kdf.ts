...
import { DERIVATION_PATH, ROOT_PUBLIC_KEY } from "./constants";

export function najPublicKeyStrToUncompressedHexPoint(): string {
    const najPublicKeyStr = ROOT_PUBLIC_KEY;
...
export async function deriveChildPublicKey(
    ...
    const scalar = await sha256Hash(
        `near-mpc-recovery v0.1.0 epsilon derivation:${signerId},${DERIVATION_PATH}`
    );
